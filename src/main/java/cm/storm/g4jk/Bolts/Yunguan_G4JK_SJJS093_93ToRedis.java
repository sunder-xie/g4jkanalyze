		...
		if(tdate.length()>=23&&imsi!=null&&imsi.length()>=15){
			...
			if(intsid!=null&&intsid.trim().equals("")==false&&intsid.trim().equals("none")==false){
				...
				if(appvalue!=null&&appvalue.length()>0){
					offset=appvalue.indexOf("#");
				}
			}else if(appid!=null&&appid.trim().equals("")==false&&appid.trim().equals("none")==false){
				key="ref_wtag_"+appid;
				appvalue=redisserver.get(key);//获取对应的用户标签信息
				if(appvalue!=null&&appvalue.length()>0){
					offset=appvalue.indexOf("#");
...
//if(tdate.length()>=23&&chinesewords!=null&&chinesewords.length()>=2&&flag==false){
...
//	//字段24，访问域名
//	g4jkbasic4gbean.setIntsid(tuple.getStringByField(Yunguan_G4JK_Basic4GFields.HOST));
//}
